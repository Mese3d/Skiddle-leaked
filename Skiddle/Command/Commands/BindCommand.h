...
public:
    ...
    BindCommand() : Command("bind", "Bind a module to a key", {
        "b"
        })
    {}
    ...
    bool execute(std::vector<std::string> cmd) override // bro has no error handling with chat messages imagineeee
    {
        ...
        {
            mod->setKeybind(0);
            ChatUtil::sendMessage(combine("Bound ", BOLD, mod->getName(), RESET," to NONE."));
            return true;
        }
        ...
        for (int i = 0; i < 193; i++)
        {
            ...
            {
                mod->setKeybind(i);
                ChatUtil::sendMessage(combine(BOLD, mod->getName(), RESET, " is now bound to ", keyNames[i], "."));
                return true;
            }
        }
