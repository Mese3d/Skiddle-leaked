...
public:
    ...
    void addParticles(int numParticles, float startX, float startY, float distance, float size, float brightness, float seconds = 4.f, float opacity = 1.f)
    {
        for (int i = 0; i < numParticles; i++)
        {
            Particle p;
            ...
            p.lifetime = seconds;
            p.opacity = opacity;
            p.color = ColorUtil::getClientColor(5, 1, brightness, p.x * 45);
            p.size = size + rand() % ((int)size / 2 + 1);
            p.distance = distance;
            particles.push_back(p);
        }
