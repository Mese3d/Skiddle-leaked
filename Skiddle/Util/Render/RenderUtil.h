...
public:
	...
	inline static void setCTX(MinecraftUIRenderContext* context)
	{
		...
		screenContext2d = reinterpret_cast<__int64*>(renderContext->ScreenContext)[2];
	}
	...
	inline void tess__begin(Tessellator* tess, int vertexFormat, int numVerticesReserved) {
		__int64 tessellator = reinterpret_cast<__int64>(tess);

		using tess_begin_t = void(__fastcall*)(Tessellator*, int, int);
		static tess_begin_t tess_begin = reinterpret_cast<tess_begin_t>(findSig("48 89 5C 24 ?? 55 48 83 EC ?? 80 B9 ?? ?? ?? ?? 00 45"));
		tess_begin(tess, vertexFormat, numVerticesReserved);
	}

	static void drawCircleFilled(Vector2<float> pos, Vector2<float> radius, UIColor color, double quality) {
		float x;
		float y;
		...
		for (double i = 0; i <= 360;) {
			...
			tess_vertex(tess, x + pos.x, y + pos.y, 0);
			i = i + quality;
			x = radius.x * (float)cos(i);
			y = radius.y * (float)sin(i);
			tess_vertex(tess, x + pos.x, y + pos.y, 0);
			tess_vertex(tess, pos.x, pos.y, 0);
			i = i + quality;
		}

		meshHelper_renderImm(screenContext2d, tess, uiMaterial);
	}
