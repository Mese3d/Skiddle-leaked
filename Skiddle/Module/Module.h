...
public:
		...
		visible_(true)
	{
		...
		registerBoolSetting("Visible", "Visible in the arraylist", &visible_);
	}
	...
	void toggle()
	{
		...
		{
			Game::Core::noti.addNotification("Toggled", name_, 1.5f);
		}

		if (Game::Core::togglesound) 
		{
			switch (Game::Core::soundtype) {
			case 0: // Click
				Util::systemPlay("Sounds\\ToggleSounds\\Click.wav");
				break;
			case 1: // Pop
				Util::systemPlay("Sounds\\ToggleSounds\\Pop.wav");
				break;
			case 2: // Sigma
				if (enabled_) {
					Util::systemPlay("Sounds\\ToggleSounds\\Sigma_Enable.wav");
				}
				else {
					Util::systemPlay("Sounds\\ToggleSounds\\Sigma_Disable.wav");
				}
				break;
			}
		}
	...
	void registerFloatSetting(const std::string name, const std::string description, float* value, float min, float max)
	{
		settings_.emplace_back(new Setting(name, description, value, min, max));
	}

	void registerIntSetting(const std::string name, const std::string description, int* value, int min, int max)
	{
		settings_.emplace_back(new Setting(name, description, value, min, max));
	}

	void registerBoolSetting(const std::string name, const std::string description, bool* state)
	{
		settings_.emplace_back(new Setting(name, description, state));
	}

	void registerEnumSetting(const std::string name, const std::string description, std::vector<std::string> enums, int* iterator)
	{
	...
	// frame events
	virtual void onEvent(class AttackEvent* event) {};
	...
	virtual void onEvent(class GetTimeofDayEvent* event) {};
	...
	virtual void onEvent(class StartDestroyBlockEvent* event) {};
